...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/98d29240bfb9e6d71eb90.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: '𝗷𝗼𝗶𝗻 𝗴𝗿𝗼𝘂𝗽 𝗯𝗼𝘁', body: wm, sourceUrl: 'https://chat.whatsapp.com/GRigRq6LkoMDR7HOwkQAIO', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
